...
import {
  dragAndPushElement,
  removeElementFromDropbox,
  hideElementFromDropbox,
  updateElementsPosition,
} from "../../../../store/draggedElementsSlice";
...
const DragAndDrop = () => {
  ...
  const filteredDraggedElementsUsingReduxStore = Elements?.filter((item1) =>
    theDraggedElements.some((item2) => item1.id === item2.id)
  );
  ...
  const theElementToDesign = Elements.filter(
    (res) => res.id === elementToDesignId && elementToDesignId
  );

  const [announcementElementText, setAnnouncementElementText] = useState(
    "50% off New Arrivals"
  );
...
`;
          ...
          >
            {selectedElement.data.animationType === "Left To Right" ? (
              <MovingAnnouncementLeftToRightSpan
                style={{ color: `${announcementELement?.foregroundColor}` }}
              >
  ...
  const announcementElementProperty = theElementToDesign?.map((res, ind) => (
  ...
  const getElementPos = (id) =>
    theDraggedElements.findIndex((ele) => ele.id === id);
          ...
          <div class="dnd-column dnd-column1">
            <div className="dnd-column1-main-div">
              <div className="dnd-design-block-header-div">
                ...
                <div>
                  <p className="dnd-header-second-text">
                    Drag, drop max 20 blocks per design
                  </p>
                </div>
              </div>
            ...
            <div className="drop-box" ref={drop}>
              <div className="scrolable-mobile-view">
                <DndContext
                  ...
                  onDragEnd={handleMobileViewElementDragEnd}
                >
                  <SortableContext
                    items={theDraggedElements && theDraggedElements}
                        ...
                        >
                          ...
                          {hoveredElementId === elements.id && (
                            <ElementActionButtons
                              hideOnClick={(event) =>
                                hideElement(event, elements.id)
                              }
                              removeOnClick={(event) =>
                                removeElement(event, elements.id)
                              }
                            />
                          )}
                        </div>
